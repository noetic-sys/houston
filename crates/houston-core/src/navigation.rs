...
impl AppState {
    ...
    pub fn filter_repos(&mut self) {
        ...
        self.repo_list_state.select((!self.filtered_repos.is_empty()).then_some(self.selected_repo));
    }
    ...
    pub fn previous_repo(&mut self) {
        if !self.filtered_repos.is_empty() {
            self.selected_repo = self.selected_repo.checked_sub(1).unwrap_or(self.filtered_repos.len() - 1);
            self.repo_list_state.select(Some(self.selected_repo));
            self.needs_action_reload = true;
        }
    }
    ...
    pub fn previous_action(&mut self) {
        if !self.actions.is_empty() {
            self.selected_action = self.selected_action.checked_sub(1).unwrap_or(self.actions.len() - 1);
            self.actions_list_state.select(Some(self.selected_action));
        }
    }
        ...
        if let Some(dialog) = &mut self.dialog {
            if let DialogType::Input { fields, focus, .. } = &mut dialog.dialog_type {
                *focus = match *focus {
                    ...
                    DialogFocus::ConfirmButton if !fields.is_empty() => DialogFocus::Field(fields.len() - 1),
                    DialogFocus::ConfirmButton => DialogFocus::CancelButton,
                    DialogFocus::CancelButton => DialogFocus::ConfirmButton,
                };
            }
    ...
    pub fn dialog_navigate(&mut self, direction: i32) {
        if let Some(dialog) = &mut self.dialog {
            match &mut dialog.dialog_type {
                DialogType::Input { .. } => {
                    if direction > 0 { self.dialog_next_field(); } else { self.dialog_prev_field(); }
                }
                DialogType::DropdownSelection { filtered_options, selected, scroll_offset, .. } => {
                    if filtered_options.is_empty() { return; }
            ...
            match &mut dialog.dialog_type {
                ...
                DialogType::DropdownSelection { search_query, options, filtered_options, selected, scroll_offset, .. } => {
                    ...
                    *filtered_options = if query.is_empty() {
                        ...
                        options.iter().filter(|o| o.to_lowercase().contains(&query)).cloned().collect()
                    };
                    *selected = 0;
                    *scroll_offset = 0;
                }
